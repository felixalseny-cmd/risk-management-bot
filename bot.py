...
async def handle_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    elif data == "save_to_portfolio":
        return await save_calculation_to_portfolio(update, context)
...
@log_performance
async def save_calculation_to_portfolio(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Save current calculation to portfolio"""
    query = update.callback_query
    await query.answer()
    ...
    instrument_type = user_data.get('instrument_type', 'forex')
    currency_pair = user_data.get('currency_pair', 'Unknown')
    direction = user_data.get('direction', 'BUY')
    entry_price = user_data.get('entry_price', 0)
    stop_loss = user_data.get('stop_loss', 0)
    take_profits = user_data.get('take_profits', [])
    risk_percent = user_data.get('risk_percent', 0.02) * 100
    ...
    trade_data = {
        ...
        'risk_percent': risk_percent,
        'status': 'planned',
        'calculation_data': user_data.copy()
    }
    ...
    await query.edit_message_text(
        "✅ *Расчет сохранен в портфель!*\n\n"
        "Теперь вы можете отслеживать эту сделку в разделе 'Мой портфель'.",
        parse_mode='Markdown'
    )
...
async def bot_main():
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            ...
            CallbackQueryHandler(handle_main_menu, pattern='^(pro_calculation|quick_calculation|portfolio|analytics|pro_info|main_menu|save_to_portfolio)$')
        ],
        states={
            MAIN_MENU: [
                CallbackQueryHandler(handle_main_menu, pattern='^(pro_calculation|quick_calculation|portfolio|analytics|pro_info|main_menu|save_to_portfolio)$')
...
def run_http_server():
    ...
    # Create new event loop for HTTP server
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    
    try:
        runner = loop.run_until_complete(start_http_server(port))
...
if __name__ == '__main__':
    ...
    if webhook_url and "render.com" in webhook_url:
        logger.info("🎯 Detected Render environment - starting both HTTP server and bot...")
        ...
        # Run HTTP server in main thread (this will block)
        run_http_server()
    else:
        logger.info("🔧 Local development environment - starting bot only...")
